...
/// 用于给定的数据是否被 [CustomDragTarget] 接受.
///
/// Used by [CustomDragTarget.onWillAccept].
typedef bool CustomDragTargetWillAccept<T>(T data);

/// 使 [CustomDragTarget] 接收数据的回调.
///
/// Used by [CustomDragTarget.onAccept].
typedef void CustomDragTargetAccept<T>(T data);

// Signature for building children of a [CustomDragTarget].
///
/// The `candidateData` argument contains the list of drag data that is hovering
/// over this [CustomDragTarget] and that has passed [CustomDragTarget.onWillAccept]. The
/// `rejectedData` argument contains the list of drag data that is hovering over
/// this [CustomDragTarget] and that will not be accepted by the [CustomDragTarget].
///
/// Used by [CustomDragTarget.builder].
typedef Widget CustomDragTargetBuilder<T>(BuildContext context, List<T> candidateData, List<dynamic> rejectedData);

/// Signature for when a [CustomDraggable] is dropped without being accepted by a [CustomDragTarget].
///
/// Used by [CustomDraggable.onCustomDraggableCanceled].
typedef void CustomDraggableCanceledCallback(Velocity velocity, Offset offset);

/// Signature for when a [CustomDraggable] leaves a [CustomDraggable].
///
/// Used by [CustomDraggable.onLeave].
typedef void CustomDragTargetLeave<T>(T data);
...
/// Where the [CustomDraggable] should be anchored during a drag.
enum DragAnchor {
...
///
///  * [CustomDragTarget]
///  * [LongPressCustomDraggable]
class CustomDraggable<T> extends StatefulWidget {
  ...
  const CustomDraggable({
    ...
    this.onDragStarted,
    this.onDragEnd,
    this.onCustomDraggableCanceled,
  ...
  final CustomDraggableCanceledCallback onCustomDraggableCanceled;

  /// Called when the MyDraggable is dropped and accepted by a [CustomDragTarget].
  ///
  /// This function might be called after this widget has been removed from the
  /// tree. For example, if a drag was in progress when this widget was removed
  ...
  @override
  CustomDraggableState<T> createState() => new CustomDraggableState<T>();
}
/// Makes its child MyDraggable starting from long press.
class LongPressCustomDraggable<T> extends CustomDraggable<T> {
  /// Creates a widget that can be dragged starting from long press.
  ///
  /// The [child] and [feedback] arguments must not be null. If
  /// [maxSimultaneousDrags] is non-null, it must be non-negative.
  const LongPressCustomDraggable({
    Key key,
    ...
    CustomDraggableCanceledCallback onCustomDraggableCanceled,
    ...
    onCustomDraggableCanceled: onCustomDraggableCanceled,
    onDragEnd: onDragEnd,
...
class CustomDraggableState<T> extends State<CustomDraggable<T>> {
  DragAvatar avatar;
  @override
  void initState() {
    super.initState();
  ...
  DragAvatar<T> _startDrag(Offset position) {
    ...
    final DragAvatar<T> avatar = DragAvatar<T>(
        ...
        onDragEnd: (Velocity velocity, Offset offset, bool wasAccepted) {
          ...
          if (!wasAccepted && widget.onCustomDraggableCanceled != null)
            widget.onCustomDraggableCanceled(velocity, offset);
        }
    );
    this.avatar = avatar;
...
///
/// When a CustomDraggable is dragged on top of a drag target, the drag target is
/// asked whether it will accept the data the CustomDraggable is carrying. If the user
/// does drop the CustomDraggable on top of the drag target (and the drag target has
/// indicated that it will accept the CustomDraggable's data), then the drag target is
/// asked to accept the CustomDraggable's data.
///
/// See also:
///
///  * [CustomDraggable]
///  * [LongPressCustomDraggable]
class CustomDragTarget<T> extends StatefulWidget {
  ...
  final CustomDragTargetBuilder<T> builder;
  ...
  /// leaves the target.
  final CustomDragTargetWillAccept<T> onWillAccept;

  /// Called when an acceptable piece of data was dropped over this drag target.
  final CustomDragTargetAccept<T> onAccept;

  /// Called when a given piece of data being dragged over this target leaves
  /// the target.
  final CustomDragTargetLeave<T> onLeave;

  @override
  _CustomTargetState<T> createState() => _CustomTargetState<T>();
}
...
class _CustomTargetState<T> extends State<CustomDragTarget<T>> {
...
class DragAvatar<T> extends Drag {
  ...
  _CustomTargetState<T> _activeTarget;
  final List<_CustomTargetState<T>> _enteredTargets = <_CustomTargetState<T>>[];
  Offset _position;
  Offset _lastOffset;
  OverlayEntry _entry;
  ...
  void updateDrag(Offset globalPosition) {
    ...
    final List<_CustomTargetState<T>> targets = _getDragTargets(result.path).toList();

    bool listsMatch = false;
    if (targets.length >= _enteredTargets.length && _enteredTargets.isNotEmpty) {
      listsMatch = true;
      final Iterator<_CustomTargetState<T>> iterator = targets.iterator;
    ...
    // Enter new targets.
    final _CustomTargetState<T> newTarget = targets.firstWhere((_CustomTargetState<T> target) {
  ...
  Iterable<_CustomTargetState<T>> _getDragTargets(List<HitTestEntry> path) sync* {
    // Look for the RenderBoxes that corresponds to the hit target (the hit target
    // widgets build RenderMetaData boxes for us for this purpose).
    for (HitTestEntry entry in path) {
      if (entry.target is RenderMetaData) {
        final RenderMetaData renderMetaData = entry.target;
        if (renderMetaData.metaData is _CustomTargetState<T>)
